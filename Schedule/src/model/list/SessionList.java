...
public class SessionList
{
  ...
  /**
   * A void method that adds a session to the session list. The method is checking if the Teacher and the ClassGroup are available for a new Session. If not, an exception is thrown.
   *
   * @param index   the index the session will be added to. Cannot be null.
   * @param session the Session object to be added.
   * @param room    the Room object in which the session will be booked.
   */
  ...
  /**
   * A getter method returning a list of Session objects with the same date for a chosen ClassGroup. The method sorts the Session objects based on their start times. @see the Session class.
   *
  ...
  public boolean isTeacherAvailable(Session session)
  {
    ...
    */
    ...
    {
      for (int i = 0; i
          < sessions.size(); i++)   //O(N) where N is the size of the sessions
      {
        if (sessions.get(i).getDate().equals(     //O(1)
            session.getDate()))//Another isOverlapped method possible to use    //O(1)
        {
  ...
  public static void main(String[] args)
  {
    ...
    System.out.println(
        "This is the list: " + sessionList1.getSessionsByDateAndClassGroup(
            date1, group1));
