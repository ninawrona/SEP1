...
public class ScheduleViewModel {
    ...
    public ObservableList<SessionViewModel> getListByDateAndClassGroup() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (model.getSessionsByDateAndClassGroup(dateHolder, model.getChosenClassGroup()) != null) {
                for (int j = 0; j < model.getSessionsByDateAndClassGroup(dateHolder, model.getChosenClassGroup()).size(); j++) {
                    holder.add(new SessionViewModel(model.getSessionsByDateAndClassGroup(dateHolder, model.getChosenClassGroup()).get(j)));
                }
            }
